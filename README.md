...
An Ansible role to install [Elastic's beats](https://www.elastic.co/products/beats).
...
Here is a list of default values for the available variables of this Ansible role.
...
* `beats_start_*` - These variables control that each beat is started after installation and reboot.
...
You can install beats also listed in `beats_download` directly from remote packages to fetch specific versions.
...
Here is a sample filebeat configuration sending logs to a Logstash instance running on 'elk' server with IP address 192.168.0.1.
We add the timezone information from the remote server in an environment variable and send it to logstash in a field named `timezone`.
